...
def test_patch_episode():
    ...
    res = requests.patch(f"{API_URL}/watch-history/collection/anime/23d5d8c1-2ab0-5279-a501-4d248dc9a63c/episode/10", json={"rating": 2, "overview": "My overview", "review": "My review"}, headers=BASE_HEADERS)
    assert res.status_code == 204
    time.sleep(1)

    res = requests.get(f"{API_URL}/watch-history/collection/anime/23d5d8c1-2ab0-5279-a501-4d248dc9a63c/episode/10", headers=BASE_HEADERS)
    assert res.status_code == 200
